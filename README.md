...
`WAITING_ON_TASK` is the matching wait-target signal. It points at the first concrete review-gated, human-blocked, or dependency-waiting task behind the current `WAITING_ON` cause, so humans can see both the wait type and the exact task involved without scanning deeper sections.
